((customElements, Joomla) => {
  ...
  window.document.addEventListener('onAgosmFileSelected', (e) => {
  ...
  /**
   * Create and dispatch onAgosmFileSelected Event
   *
  ...
  Joomla.getImage = (data, editor, fieldClass) => {
    return new Promise((resolve, reject) => {
      const apiBaseUrl = `${Joomla.getOptions('system.paths').rootFull}administrator/index.php?option=com_agosm&format=json`;
  ...
  class JoomlaFieldAgosm extends HTMLElement {
    ...
    updatePreview() {
      ...
      if (this.preview) {
        ...
        const div = this.querySelector('.field-agosm-preview');

        if (!value) {
          div.innerHTML = '<span class="field-agosm-preview-icon fa fa-picture-o"></span>';
        } else {
          div.innerHTML = '';
  ...
  customElements.define('joomla-field-agosm', JoomlaFieldAgosm);
})(customElements, Joomla);
